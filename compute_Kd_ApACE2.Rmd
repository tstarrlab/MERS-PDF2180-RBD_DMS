---
title: "Compute per-barcode ApACE2 binding affinity"
author: "Tyler Starr"
date: "4/26/2024"
output:
  github_document:
...
This notebook reads in per-barcode counts from `count_variants.ipynb` for ApACE2-binding Tite-seq experiments, computes functional scores for RBD ApACE2-binding affiniity, and does some basic QC on variant binding functional scores.
...
We successfully generated *K*<sub>D</sub> estimates for `r sum(!is.na(dt[library=="pool3",Kd_ApACE2]))` of our pool3 barcodes (`r round(sum(!is.na(dt[library=="pool3",Kd_ApACE2]))/nrow(dt[library=="pool3",])*100,digits=2)`%), and `r sum(!is.na(dt[library=="pool4",Kd_ApACE2]))` of our pool4 barcodes (`r round(sum(!is.na(dt[library=="pool4",Kd_ApACE2]))/nrow(dt[library=="pool4",])*100,digits=2)`%). 
...
```{r binding_distribution_vioplo_DMSt, echo=T, fig.width=6, fig.height=4.5, fig.align="center", dpi=300,dev="png"}
...
We have generated binding measurements for `r round(nrow(dt[!is.na(log10Ka_ApACE2)])/nrow(dt)*100,digits=2)`% of the barcodes in our libraries.
