...
This notebook reads in the per-barcode titration Kds, expression, and serum or mAb binding values from relevant scripts. It synthesizes these two sets of results and calculates the final 'mean' phenotypes for each variant, and generates some coverage and QC analyses.
...
Calculate the mean binding and expression score collapsed by genotype -- simple mean for log10Ka_receptor, log(expr) and serum AUC values, need to do mean(log10(EC50))for mAbs. Also output the number of barcodes across which a variant score was determined in each library.
...
Some QC plots. First, for DMS libs, look at distribution of number barcodes for binding and expression measurements for single mutant detemrinations. These are 'left-justified' histograms, so the leftmost bar represents the number of genotypes for which no barcodes were collapsed to final measurement in a pool.

Note, for many of these plots, I exclude A106P in MERS_rpk libs because we saw from pacbio it has overrepresentation among variants and so dominates the x-axis
...
Format/break up tables into (i) a 'mutation lookup table', where we focus just on the single mutants (and wildtype) in the DMS libraries, breakup the string of mutations, and fill in the table to also include any missing mutants, and (ii) panmerbeco pool of wildtype values. We'll do each all the way through visualization below under major headings.
...
We have duplicates for expr, sera, and MERS4 measurement. Let's look at correlations!
...
Relationship between mutaitonal effects on binding and expression as early view how the different phenotypes are influenced by expressione ffects.

```{r plot_correlations_bind-v-expr, echo=T, fig.width=12, fig.height=12, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(3,3))
#MERS_rpk, hDPP4
...
My quick interpretations:
 - MERS_rpk helps a lot compared to our prior MERS data! for example, expression-enhancing mutations don't deterministically drive higher hDPP4 affinities. For receptor-bindign data in the MERS_rpk background, I think we could do expression normalizing on the expression-enhancing end of the spectrum. For PDF-2180, there remains broader between expression and ApACE2 binding, so we will probalby need to come up with a stronger expression-normalized metric. Might be better to still only apply above a certain range as we don't really need to 'boost' the delta_bind for stronger affinity- and expression-decreasing muts in either dataset
 - for mAbs and serum, we'll apply some filter for escape muts with expression less than ~-0.75
 - for serum, like we saw with sarbeco, we'll probably need to normalize by the global correlation between serum-binding and expression above this -0.75 threshold
 - for mAbs, looks a bit more complex with variation among the mAbs the degree to which there is no relationship past the threshold (e.g., LCA60), correlation in one part of the range but then stops around ~-0.5 (MERS4), or consistent correlation with two different shapes (4C2, S41). For now, can probably jsut apply same treatemnt as sera, b/c normalizaiton will be mild for e.g. LCA60


Apply some normalizations and filters

```{r normalize_expr, echo=T, fig.width=12, fig.height=12, fig.align="center", dpi=300,dev="png"}
...
```{r plot_correlations_bind-v-expr_normalized, echo=T, fig.width=12, fig.height=12, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(3,3))
#MERS_rpk, hDPP4
...
```{r censor_n_barcodes_libs_1, echo=T, fig.width=8, fig.height=4, fig.align="center", dpi=300,dev="png"}
# min_bc <- 2
# min_lib <- 2
# 
...
```{r order_plotting_factors_MERS_rpk}
...
Make heatmaps for MERS, showing raw affinity and delta-affinity for hDPP4 binding

```{r heatmap_DMS_log10Ka_hDPP4, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka-hDPP4-by-target, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
...
Make heatmaps for MERS, showing raw expression and delta-expression of muts relative to respective wildtype

```{r heatmap_DMS_expression-MERS_rpk, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-expression_MERS_rpk, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
...
Make heatmaps for MERS, showing delta-logEC50 for each of the four mAbs. Can I get these stacked?

Illustrate delta_log10EC50 for mAbs faceted

```{r heatmap_DMS_delta-log10Ka-mAbs-by-target_MERS_rpk, fig.width=25,fig.height=12,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_MERS_rpk_delta-AUC-serum-by-target, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r order_plotting_factors_PDF2180}
#order mutant as a factor for grouping by rough biochemical grouping
...
Make heatmaps for PDF2180, showing raw affinity and delta-affinity for ApACE2 binding

```{r heatmap_DMS_log10Ka_ApACE2, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_DMS_delta-log10Ka-ApACE2-by-target, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
...
Make heatmaps for PDF2180, showing raw expression and delta-expression of muts relative to respective wildtype

```{r heatmap_DMS_expression-PDF2180, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
p1 <- ggplot(temp[measurement=="expr",],aes(site_MERS,mutant))+geom_tile(aes(fill=value),color="black",lwd=0.1)+
  #scale_fill_gradientn(colours=c("#FFFFFF","#003366"),limits=c(5.5,9),na.value="yellow")+
  scale_fill_gradientn(colours=c("#FFFFFF","#FFFFFF","#003366","#003366"),limits=c(4.5,8.5),values=c(0,1/4,3.5/4,4/4),na.value="yellow")+ #three notches in case I want to 'censor' closer to the 5 boundary condition
  #scale_x_continuous(expand=c(0,0),breaks=c(377,seq(380,591,by=5)))+
...
```{r heatmap_DMS_delta-expression_PDF2180, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
p1 <- ggplot(temp[measurement=="delta_expr",],aes(site_MERS,mutant))+geom_tile(aes(fill=value),color="black",lwd=0.1)+
  scale_fill_gradientn(colours=c("#A94E35","#A94E35","#F48365","#FFFFFF","#7378B9","#383C6C","#383C6C"),
                       limits=c(-2.5,1.5),
                       values=c(0/4,1/4,1.75/4,2.5/4,3/4,3.5/4,4/4),
...
```{r format_variant_tables_panmerbeco}
#hDPP4
...
```{r plot_correlations_panmerbeco, echo=T, fig.width=8, fig.height=8, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,2))
#expr
...
```{r plot_rpk, echo=T, fig.width=8, fig.height=8, fig.align="center",dpi=300,dev="png"}
par(mfrow=c(3,3))
#expr
...
Looks like moving forward, we can go ahead with the 'rpk' versions for all plots?

Relationship between binding phenotypes and expression as early view how the different phenotypes are influenced by expressione ffects.

```{r plot_correlations_bind-v-expr_panmerbeco, echo=T, fig.width=12, fig.height=12, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(3,3))
#hDPP4
...
My quick interpretations: shapes look similar to the DMS libraries, obviously just a bit lighter b/c more sparse dataset.

I think we only need to really normalize the serum values, and in that we can do what we've done previously which is actually normalize on the absolute AUC range, not just on the delta. Let's also collapse to a <=2 category

```{r normalize_expr_panmerbeco, echo=T, fig.width=12, fig.height=12, fig.align="center", dpi=300,dev="png"}
...
Plots before and after normalization metrics (only normalizing for serum)

```{r plot_correlations_bind-v-expr_normalized_panmerbeco, echo=T, fig.width=8, fig.height=4, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(1,2))
#serum, no norm
...
```{r censor_n_barcodes_libs_2, echo=T, fig.width=8, fig.height=4, fig.align="center", dpi=300,dev="png"}
# min_bc <- 2
# min_lib <- 2
# 
# dt_final[n_bc_bind < min_bc & n_libs_bind < min_lib, c("bind","delta_bind","n_bc_bind","n_libs_bind") := list(NA,NA,NA,NA)]
...
```{r n_barcode_plots_2, echo=T, fig.width=7, fig.height=5, fig.align="center", dpi=300,dev="png"}
# par(mfrow=c(2,2))
# #MERS
...
```{r order_plotting_factors_panmerbeco}
...
```{r heatmap_panmerbeco_log10Ka_hDPP4, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_panmerbeco_expression, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
...
```
Make heatmaps for panmerbeco, showing logEC50 for each of the four mAbs. Can I get these stacked?

Illustrate log10EC50 for mAbs faceted

```{r heatmap_panmerbeco_mAbs, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_panmerbeco_serum, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmap_panmerbeco_serum_norm, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r order_plotting_factors_panmerbeco_rpk}
#order variants as factors based on phylogenetic relationships
#make temp long-form data frame
...
```{r heatmap_panmerbeco-rpk-extant_log10Ka_hDPP4, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
p1 <- ggplot(temp[measurement %in% c("bind_hDPP4","bind_ApACE2"),],
             aes(target,measurement))+geom_tile(aes(fill=value),color="black",lwd=0.1)+
  #scale_fill_gradientn(colours=c("#FFFFFF","#003366"),limits=c(5,10),na.value="yellow")+
  scale_fill_gradientn(colours=c("#FFFFFF","#FFFFFF","#003366"),limits=c(5,10),values=c(0,1/5,5/5),na.value="yellow")+ #effective coloring 6-10
...
```{r heatmap_panmerbeco-rpk-extant_expression, fig.width=25,fig.height=6,fig.align="center", dpi=500,dev="png",echo=T}
p1 <- ggplot(temp[measurement %in% c("expr"),],
             aes(target,measurement))+geom_tile(aes(fill=value),color="black",lwd=0.1)+
  scale_fill_gradientn(colours=c("#FFFFFF","#003366"),limits=c(5,9),na.value="yellow")+
  #scale_fill_gradientn(colours=c("#FFFFFF","#FFFFFF","#003366"),limits=c(5,10),values=c(0,1/5,5/5),na.value="yellow")+ #effective coloring 6-10
...
```{r heatmap_panmerbeco-rpk-extant_mAbs, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
p1 <- ggplot(temp[measurement %in% c("bind_LCA60","bind_S41","bind_4C2","bind_MERS4"),],
             aes(target,measurement))+
  geom_tile(aes(fill=value),color="black",lwd=0.1)+
  #scale_fill_gradientn(colours=c("#003366","#FFFFFF"),limits=c(-0.5,5),na.value="yellow")+
...
```{r heatmap_panmerbeco-rpk-extant_serum, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
p1 <- ggplot(temp[measurement %in% c("bind_serum"),],
             aes(target,measurement))+
  geom_tile(aes(fill=value),color="black",lwd=0.1)+
  scale_fill_gradientn(colours=c("#FFFFFF","#003366"),limits=c(0.5,6),na.value="yellow")+
...
```{r heatmap_panmerbeco-rpk-extant_serum_norm, fig.width=25,fig.height=10,fig.align="center", dpi=500,dev="png",echo=T}
p1 <- ggplot(temp[measurement %in% c("bind_serum_norm"),],
             aes(target,measurement))+
  geom_tile(aes(fill=value),color="black",lwd=0.1)+
  scale_fill_gradientn(colours=c("#FFFFFF","#003366"),limits=c(2,6),na.value="yellow")+
