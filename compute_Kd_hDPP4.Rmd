...
We successfully generated *K*<sub>D</sub> estimates for `r sum(!is.na(dt[library=="pool1A",Kd_hDPP4]))` of our pool1A barcodes (`r round(sum(!is.na(dt[library=="pool1A",Kd_hDPP4]))/nrow(dt[library=="pool1A",])*100,digits=2)`%), and `r sum(!is.na(dt[library=="pool2A",Kd_hDPP4]))` of our pool2A barcodes (`r round(sum(!is.na(dt[library=="pool2A",Kd_hDPP4]))/nrow(dt[library=="pool2A",])*100,digits=2)`%). 
