---
title: "Compute per-barcode hDPP4 binding affinity"
author: "Tyler Starr"
date: "12/30/2022"
output:
  github_document:
...
This notebook reads in per-barcode counts from `count_variants.ipynb` for hDPP4-binding Tite-seq experiments, computes functional scores for RBD hDPP4-binding affiniity, and does some basic QC on variant binding functional scores.
...
```
Note, we currently have turned off the filtering to throw out bimodal mean bin observations
...
We successfully generated *K*<sub>D</sub> estimates for `r sum(!is.na(dt[library=="pool1",Kd_hDPP4]))` of our pool1 barcodes (`r round(sum(!is.na(dt[library=="pool1",Kd_hDPP4]))/nrow(dt[library=="pool1",])*100,digits=2)`%), and `r sum(!is.na(dt[library=="pool2",Kd_hDPP4]))` of our pool2 barcodes (`r round(sum(!is.na(dt[library=="pool2",Kd_hDPP4]))/nrow(dt[library=="pool2",])*100,digits=2)`%). 
...
```{r Kd_distribution, fig.width=5, fig.height=6, fig.align="center",dpi=300,dev="png"}
par(mfrow=c(2,1))
...
```{r nMSR_v_cell_count, fig.width=8, fig.height=4, fig.align="center",dpi=300,dev="png"}
...
```{r binding_distribution_vioplo_DMS, echo=T, fig.width=6, fig.height=4.5, fig.align="center", dpi=300,dev="png"}
...
```{r binding_distribution_vioplot_panmerbeco, echo=T, fig.width=46, fig.height=4.5, fig.align="center", dpi=300,dev="png"}
...
We have generated binding measurements for `r round(nrow(dt[!is.na(log10Ka_hDPP4)])/nrow(dt)*100,digits=2)`% of the barcodes in our libraries.
